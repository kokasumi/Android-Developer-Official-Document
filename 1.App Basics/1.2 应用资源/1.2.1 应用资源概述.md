...
```mermaid
graph TB
	opt1(1.剔除与设备配置相冲突的限定符资源文件夹)
	opt2("2.确定下一个进行对比的限定符(按照限定符优先级进行确定)")
	cond1{3.是否存在资源文件夹使用该限定符?}
	opt3(4.剔除不包含该限定符的资源文件夹)
	
	opt1 --> opt2
	opt2 --> cond1
	cond1--否--> opt2
	cond1--是--> opt3
	opt3--5.循环执行直至只剩下最后一个匹配的资源文件--> opt2
```

```flow
opt1=>operation: 1.剔除与设备配置相冲突的限定符资源文件夹
opt2=>operation: 2.确定下一个进行对比的限定符(按照限定符优先级进行确定)
cond1=>condition: 3.是否存在资源文件夹使用该限定符?
opt3=>operation: 4.剔除不包含该限定符的资源文件夹
cond2=>condition: 5.是否只剩下最后一个匹配资源?
ed=>end: 结束
