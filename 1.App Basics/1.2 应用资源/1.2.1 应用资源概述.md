...
- 创建`drawable-en-rCA `和`drawable-fr-rCA `两个替代资源文件夹，使用`<bitmap>`标签创建一个指向`icon_ca.png`的drawable资源，并将资源文件命名为`icon.xml`。
...
- 在`values-en-rCA `和`values-fr-rCA `文件夹中新建`drawables.xml `资源文件，并使用`<drawable>`标签新建`icon_ca.png`资源文件的别名。
